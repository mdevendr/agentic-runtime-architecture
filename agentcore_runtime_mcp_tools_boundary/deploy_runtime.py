...
ZIP_PATH = BUILD_DIR / "agentcore_mcp_tools_boundary.zip"
DEFAULT_RUNTIME_NAME = "mcp_tools_boundary"
RUNTIME_NAME_PATTERN = re.compile(r"^[a-zA-Z][a-zA-Z0-9_]{0,47}$")
ROLE_POLICY_NAME = "AgentCoreMcpToolsBoundaryPolicy"
...
def default_code_bucket(account_id: str, region: str) -> str:
    return f"agentcore-mcp-boundary-{account_id}-{region}"

def default_role_name(region: str) -> str:
    return f"AmazonBedrockAgentCoreMcpToolsBoundary-{region}"
...
def ensure_execution_role(
    ...
    try:
        ...
        role = iam.create_role(
            ...
            Description="Execution role for the AgentCore MCP tools boundary runtime.",
            Tags=[
                {
                    "Key": "ArchitecturePattern",
                    "Value": "AgentCoreMcpToolsBoundary",
                }
            ],
        )["Role"]
...
def create_zip(skip_deps: bool = False) -> Path:
    ...
    with zipfile.ZipFile(ZIP_PATH, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.write(BASE_DIR / "mcp_order_server.py", "mcp_order_server.py")
        archive.write(BASE_DIR / "mcp_refund_server.py", "mcp_refund_server.py")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Deploy the AgentCore MCP tools boundary runtime.")
    ...
    key = f"{runtime_name}/agentcore_mcp_tools_boundary.zip"
    ...
    response = control.create_agent_runtime(
        ...
        tags={
            "ArchitecturePattern": "AgentCoreMcpToolsBoundary",
        },
    )
