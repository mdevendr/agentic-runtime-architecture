...
ZIP_PATH = BUILD_DIR / "agentcore_direct_tools_baseline.zip"
DEFAULT_RUNTIME_NAME = "direct_tools_baseline"
RUNTIME_NAME_PATTERN = re.compile(r"^[a-zA-Z][a-zA-Z0-9_]{0,47}$")
ROLE_POLICY_NAME = "AgentCoreDirectToolsBaselinePolicy"
...
def default_code_bucket(account_id: str, region: str) -> str:
    return f"agentcore-dt-baseline-{account_id}-{region}"

def default_role_name(region: str) -> str:
    return f"AmazonBedrockAgentCoreDirectToolsBaseline-{region}"
...
def ensure_execution_role(
    ...
    try:
        ...
        role = iam.create_role(
            ...
            Description="Execution role for the AgentCore direct tools baseline runtime.",
            Tags=[
                {
                    "Key": "ArchitecturePattern",
                    "Value": "AgentCoreDirectToolsBaseline",
                }
            ],
        )["Role"]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Deploy the AgentCore direct tools baseline runtime.")
    parser.add_argument("--no-wait", action="store_true", help="Do not wait for READY status.")
    parser.add_argument(
        "--skip-deps",
        action="store_true",
        help="Package only main.py. Use only when dependencies are provided by another build path.",
    )
    ...
    key = f"{runtime_name}/agentcore_direct_tools_baseline.zip"
    ...
    response = control.create_agent_runtime(
        ...
        tags={
            "ArchitecturePattern": "AgentCoreDirectToolsBaseline",
        },
    )
